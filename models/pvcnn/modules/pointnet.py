...
class PointNetSAModule(nn.Module):
    ...
    def forward(self, inputs):
        features, coords, time_emb_ = inputs
        ...
        features = torch.cat(features_list, dim=1) if len(features_list) > 1 else features_list[0]
        return features, centers_coords, time_emb_
...
class PointNetFPModule(nn.Module):
    ...
    def forward(self, inputs):
        if len(inputs) == 4:
            points_coords, centers_coords, centers_features, time_emb_ = inputs
            points_features = None
        else:
            points_coords, centers_coords, centers_features, points_features, time_emb_ = inputs
        ...
        features = self.mlp(interpolated_features)
        return features, points_coords, time_emb_
